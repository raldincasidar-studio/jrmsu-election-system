    ...
    .container
        Transition(name="blur" mode="out-in")
            ...
            div(v-else)
                ...
                button(@click="submitVote()" v-if="isTrueInfoChecked && isPreviewMode") 
                  | SUBMIT VOTE
                  svg(xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6")
                    path(stroke-linecap="round" stroke-linejoin="round" d="M17.25 8.25 21 12m0 0-3.75 3.75M21 12H3")
...
async function getCandidates() {
  ...
  positionsWithCandidates.value = Object.entries(candidatesByPosition).map(([position, {max_votes, candidates}]) => ({
    position,
    max_votes,
    candidates,
    selectedCandidates: []
...
async function selectCandidate(position_index, candidate) {
  ...
  if (candidateIndex !== -1) {
    ...
    if (positionsWithCandidates.value[position_index].selectedCandidates.length >= positionsWithCandidates.value[position_index].max_votes) {
      $toast.fire({
        title: 'Max Votes Reached. Unselect another candidate to select this candidate',
        icon: 'error'
      })
      return;
    }
